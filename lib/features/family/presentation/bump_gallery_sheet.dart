...
class BumpGallerySheet extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // Используем обновленный провайдер и стрим, который сам собирает пути
    final snapshotsAsync = ref.watch(bumpRepositoryProvider).watchSnapshots();
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          // Drag Handle (визуальный элемент для смахивания)
          Container(
              width: 40, height: 4,
              decoration: BoxDecoration(
          ...
          Expanded(
            child: StreamBuilder<List<BumpSnapshot>>(
              stream: snapshotsAsync,
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                }

                // Мапим существующие фото по неделям для быстрого доступа
                final existingPhotos = {for (var e in snapshot.data ?? []) e.week: e};
  ...
  Widget _buildPhotoCard(BuildContext context, WidgetRef ref, int week, BumpSnapshot? photo, AppLocalizations l10n) {
    final theme = Theme.of(context);
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: isTaken
          ...
          fit: StackFit.expand,
          children: [
            // ВАЖНО: Используем tempFullFilePath, который собрал репозиторий
            Image.file(
          ...
          children: [
            Icon(Icons.add_a_photo_outlined, color: primaryColor.withOpacity(0.5), size: 32),
            const SizedBox(height: 8),
            Text(
                l10n.galleryWeek(week),
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                final xFile = await picker.pickImage(source: ImageSource.camera, maxWidth: 1200);
                if (xFile != null) {
                  // Вызываем обновленный метод сохранения
                  await ref.read(bumpRepositoryProvider).savePhoto(week, xFile.path);
                }
              },
            ),
            ListTile(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              // Удаление по ID через провайдер
              ref.read(bumpRepositoryProvider).deletePhoto(photo.id);
              Navigator.pop(context);
            },
